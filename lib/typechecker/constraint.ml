...
let rec to_latex = function
  | Pred p -> P.to_string p
  | Conj (l, r) ->
      let l_str, r_str = Misc.proj2 to_latex l r in
      Printf.sprintf "(%s \\land %s)" l_str r_str
  | Impl { x; base; p; c } ->
      let x_str = Ident.to_string x in
      let b_str = Base_ty.to_string base in
      let p_str = P.to_string p in
      let c_str = to_latex c in
      Printf.sprintf "\\forall %s\\colon %s.%s \\implies %s" x_str b_str p_str
        c_str
